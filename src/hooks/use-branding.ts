...
const normalizeBranding = (value: unknown): BrandingSettings => {
  ...
  let panel_name = typeof obj.panel_name === "string" && obj.panel_name.trim() ? obj.panel_name.trim() : "";
  const logo_url = typeof obj.logo_url === "string" && obj.logo_url.trim() ? obj.logo_url : null;

  // Bloquear nome legado "xSync" mas manter a logo se existir
  if (panel_name.toLowerCase() === "xsync") {
    panel_name = "";
  }

  return { logo_url, panel_name };
};
